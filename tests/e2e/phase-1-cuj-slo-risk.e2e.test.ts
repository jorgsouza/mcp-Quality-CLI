...
describe('FASE 1 E2E: CUJ → SLO → Risk Pipeline', () => {
  const testRepo = resolve(process.cwd());
  const testProduct = 'mcp-quality-cli-test-e2e';
  const qaRoot = resolve(testRepo, `qa/${testProduct}`);

  beforeAll(async () => {
    // Limpar qa/mcp-quality-cli-test-e2e antes de rodar
  ...
  afterAll(async () => {
    // Limpar após testes (comment out para debug)
    // if (existsSync(qaRoot)) {
    //   await rm(qaRoot, { recursive: true, force: true });
    // }
  });
